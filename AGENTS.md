# AGENTS.md — my-landing-portfolio
**Para:** OpenCode / cualquier agente IA que trabaje en este proyecto  
...
Portfolio personal y carta de presentación de Javier Rojas — Full-Stack Developer de Viña del Mar, Chile.
Stack objetivo: **Astro + Tailwind CSS + TypeScript**.
Referencia visual: `BRAND.md` en la raíz del proyecto.
Deploy target: **Vercel** (desde rama `main`).
**Package manager: `pnpm`** — nunca usar `npm` ni `yarn` en este proyecto.

---

## ARCHIVOS REQUERIDOS

Al explorar el proyecto, verificar que existan en la raíz:

- CONTEXT.md — estado actual del proyecto (crear con template si no existe)
- SPEC.md — especificación funcional (crear con template si no existe)
- BRAND.md — referencia visual (crear con template si no existe)
- README.md — overview del proyecto (generar desde el contexto del proyecto)
- .env.example — variables de entorno (crear vacío si no existe)

Si alguno falta, crearlo con un template básico antes de continuar.

---

## REGLAS INQUEBRANTABLES

El agente NUNCA viola estas reglas. Si una instrucción del usuario entra en conflicto, primero señalar el conflicto antes de proceder.

### TypeScript
- No usar `any`. Si el tipo es desconocido, usar `unknown`
- Todas las funciones y componentes Astro/TSX tienen tipos explícitos
- Interfaces para props, no `type` inline cuando la forma tiene más de 2 campos
- No usar `as` (type assertion) sin un comentario que explique por qué

### Estructura de archivos
```
src/
├── components/       → Componentes Astro (.astro) y TSX si necesitan estado
├── layouts/          → BaseLayout.astro
├── pages/            → index.astro (y rutas futuras)
├── styles/           → global.css con variables de marca
├── data/             → JSON/TS con contenido (proyectos, skills, etc.)
...
- Archivos en kebab-case: `hero-section.astro`, `project-card.astro`
- Datos en `src/data/`, no hardcodeados dentro de los componentes
- Los componentes NO hacen fetch. Los datos se pasan como props desde `index.astro`

### Astro específico
- Preferir `.astro` sobre `.tsx` salvo que el componente necesite estado del cliente (`useState`, `useEffect`)
- Cuando sea necesario estado de cliente, usar `client:load` o `client:visible` (preferir `client:visible` para secciones below-the-fold)
- No usar frameworks de terceros (React, Vue) salvo para componentes que realmente lo requieran
- Imágenes: usar el componente `<Image>` de `@astrojs/image` para optimización automática

### CSS / Tailwind
- Variables de marca en `src/styles/global.css` siguiendo `BRAND.md`
- No hardcodear colores en componentes — siempre usar tokens (`text-primary`, `bg-card`, etc.)
- Clases utilitarias de Tailwind para spacing y layout
- CSS custom solo para efectos que Tailwind no puede hacer: glitch, scanlines, neon glow, gradient mesh
- No usar `!important`

### Privacidad — CRÍTICO
- **NUNCA** incluir número de teléfono en ninguna sección de la web
- **NUNCA** incluir dirección ni ciudad en la web pública (esa info va en el CV, no aquí)
- El formulario de contacto usa Formspree — el email real de Javier **no aparece en el HTML**
- Si el usuario pide agregar datos de contacto privados, rechazar y explicar por qué

### Skills — NO USAR PORCENTAJES
- Las habilidades se muestran como **tech cards con logo + etiqueta de contexto**
- Las etiquetas válidas son: `"3+ años"`, `"producción"`, `"en aprendizaje"`, `"certificando"`, `"2+ años"`, etc.
- **Nunca** asignar porcentajes (85%, 90%) a ninguna skill — son arbitrarios y engañosos
- Los logos vienen de **Devicons** CDN o SVGs en `public/icons/`

### Certifications
- Las certificaciones van en `src/data/certifications.ts`
- Cada cert tiene: `name`, `issuer`, `status` (`"obtained"` | `"in-progress"` | `"planned"`), `date` (obtenida o estimada), `badgeUrl` (opcional), `credentialUrl` (opcional)
- El estado se refleja visualmente con badges de color: verde (obtenida), ambar pulsante (en progreso), gris (planificada)
- Se pueden agregar nuevas certs editando solo el archivo de datos — sin tocar componentes
- Todos los `<img>` tienen `alt` descriptivo
- Links con `href` real, no `href="#"` en producción
- Contraste mínimo AA entre texto y fondo
- La ilustración SVG del hero tiene `aria-label` y `role="img"`

### Performance
- No cargar fuentes desde Google Fonts en producción — descargar y servir localmente en `public/fonts/`
- Spline3D (cuando se integre): cargar con `client:visible` para no bloquear LCP
- Lazy load en imágenes de proyectos
- Sin dependencias de runtime de JS para contenido estático

### Git
- Conventional Commits: `feat:`, `fix:`, `chore:`, `docs:`, `style:`, `refactor:`
- Un commit = una cosa
- Nunca commitear `.env`, `node_modules`, o claves API
- Rama principal: `main`

### Seguridad
- Sin hardcodear tokens, API keys ni datos sensibles
- Formulario de contacto: validar en el servidor (Astro endpoint o Formspree/Resend)
- No exponer el email directamente en HTML estático — usar un servicio de formularios

---

## WORKFLOW SDD PARA ESTE PROYECTO

Antes de implementar cualquier feature nuevo (sección, componente, integración):

1. **Explore** — revisar codebase existente, convenciones ya establecidas
2. **Propose** — proponer enfoque técnico y pedir aprobación
3. **Spec** — definir qué hace exactamente el componente/feature
4. **Implement** — codear siguiendo la spec aprobada
5. **Verify** — revisar contra `AGENTS.md` y `BRAND.md`

Para cambios menores (fix de texto, ajuste de color, typo): implementar directamente.

---

## STACK Y DEPENDENCIAS APROBADAS
...
```json
{
  ...
  "typescript": "5.x",
  "ui_extras": ["lucide-astro (iconos UI)"],
  "tech_logos": "Devicons CDN (https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/)",
  "deploy": "Vercel",
  "forms": "Formspree",
  "3d_futuro": "Spline3D (embed via script, client:visible)"
}
```
...
No agregar dependencias sin proponer y justificar primero.

---

## DATOS DEL PORTFOLIO

El contenido real de proyectos, skills y bio se gestiona en `src/data/`:
...
El agente NO inventa proyectos ni niveles de skill. Si no hay datos reales, usar placeholders explícitos marcados con `TODO:`.
...
Ver `BRAND.md` para:
- Paleta de colores completa (tokens CSS)
- Tipografía (Rajdhani + JetBrains Mono)
- Efectos visuales (glitch, neon glow, grid background, gradient mesh)
- Comportamiento de animaciones (stagger, IntersectionObserver)
- Componentes UI (terminal window, botones, progress bars)
- Layout y espaciado

---

## LO QUE NO HACE EL AGENTE SIN APROBACIÓN EXPLÍCITA

- Cambiar la paleta de colores del `BRAND.md`
- Agregar secciones no especificadas en la Spec
- Integrar servicios de terceros nuevos (analytics, chat widget, etc.)
- Modificar el `AGENTS.md`
